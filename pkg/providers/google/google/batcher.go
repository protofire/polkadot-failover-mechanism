	...
	case resp := <-respCh:
		if resp.err != nil {
			return nil, fmt.Errorf(
				"request %q returned error: %w", request.DebugID, resp.err)
		}
	...
	if resp.IsError() && len(batch.subscribers) > 1 {
		...
		for _, sub := range batch.subscribers {
			...
			if singleResp.IsError() {
				singleResp.err = fmt.Errorf(
					"batch request and retried single request %q both failed. Final error: %w", sub.singleRequest.DebugID,
					singleResp.err)
			}
