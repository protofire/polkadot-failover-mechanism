...
func deleteNetworkSubnets(ctx context.Context, client *compute.Service, project, prefix string, networkNames []string, dryRun bool) error {
	...
	for _, subnet := range subnets {
		...
		go func(subnet subnetItem, wg *sync.WaitGroup) {
			...
			if err := waitForOperation(ctx, op, prepareRegionGetOp(ctx, client, project, subnet.region, op.Name)); err != nil {
...
func deleteNetworkFirewals(ctx context.Context, client *compute.Service, project, prefix string, networkNames []string, dryRun bool) error {
...
func deleteNetworks(ctx context.Context, client *compute.Service, project, prefix string, networkNames []string, dryRun bool) error {
...
func NetworkClean(project, prefix string, dryRun bool) error {
	...
	if err := deleteNetworkFirewals(ctx, client, project, prefix, networkNames, dryRun); err != nil {
		return err
	}
	if err := deleteNetworks(ctx, client, project, prefix, networkNames, dryRun); err != nil {
		return err
	}
