...
readarray -t key_names < <(aws ssm get-parameters-by-path --region "${region}" --recursive --path "/polkadot/validator-failover/${1}/keys/" | jq .Parameters[].Name | awk -F'/' '{print$(NF-1)}' | sort | uniq)
